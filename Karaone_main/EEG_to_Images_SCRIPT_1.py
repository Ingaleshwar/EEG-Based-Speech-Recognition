...
os.system('C:\\Users\\adity\\Desktop\\MDS_Kara_One-main\\MDS_SCRIPTS.py')
def main():
    PATH_TO_DATA = r'E:\MP\Major Project - ISR MATLAB Code\Major Project - ISR MATLAB Code\Raw_data'       #This should be the path to the root folder containing subject sub-folders with their Kara One data
    SUBJECTS = ['MM05']
    #, MM05, 'MM08', 'MM09', 'MM10', 'MM11', 'MM12', 'MM14', 'MM15', 'MM16', 'MM18', 'MM19', 'MM20', 'MM21', 'P02'
    ...
    for subject in mds.Dataset.registry:
        ...
        print('Loading data..')
        subject.load_data(r'E:\MP\Major Project - ISR MATLAB Code\Major Project - ISR MATLAB Code\Raw_data\\', raw=True)
        ...
        #for less than 2Hz
        #filtered.filter(0., 2., fir_design='firwin')
        #ica_data.filter(0., 2., fir_design='firwin')
        #for above 45Hz
        # filtered.filter(45., None, fir_design='firwin')
        # ica_data.filter(45., None, fir_design='firwin')
        ...
        epochs_ica = mne.Epochs(ica_data, prompts, event_id, tmin=-0.01, tmax=5.0, baseline=None, preload=True)
        ...
        # fig = ica.plot_components(show=False)  # Generates the plot
        # fig.savefig(os.path.join(save_path, 'ICA_Components.png'))  # Save the single plot
        # plt.close(fig)  # Close the figure after saving

        # # Plot the reconstructed sources (no ICA exclusions)
        # fig = ica.plot_sources(epochs_filtered, show=False)  # Generates the plot
        # fig.savefig(os.path.join(save_path, 'Filtered_Epochs_ICAcomps.png'))  # Save the plot
        # plt.close(fig)  # Close the figure

        # # Plot and save properties of the first 5 ICA components
        # for i, fig in enumerate(ica.plot_properties(epochs_filtered, picks=range(5), show=False)):
        #     name = os.path.join(save_path, f'ica_plot_properties_ICA_{i}.png')  # File name
        #     fig.savefig(name)  # Save the figure
        #     plt.close(fig) 
