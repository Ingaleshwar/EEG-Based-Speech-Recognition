...
def main(gaf, word, method, sub):
    ...
    PATH_TO_DATA = "E:\\MP\\Major Project - ISR MATLAB Code\\Major Project - ISR MATLAB Code\\data\\"       #This should be the path to the root folder containing subject sub-folders with their Kara One data
    ...
    # engine='c' delimiter="\n"
    df = pd.read_csv(PATH_TO_DATA + subject + "\\kinect_data\\labels.txt", engine='c' , header=None)
    ...
    # print(target_train_x)
    # print("rahul")
    # print(target_train_y)
    target_train_x = target_train_x._append(target_train_x.copy(), ignore_index=True)
    target_train_y = target_train_y._append(target_train_y.copy(), ignore_index=True)

    train_x = target_train_x._append(comp_train_x, ignore_index=True)
    train_y = target_train_y._append(comp_train_y, ignore_index=True)

    test_x = target_test_x._append(comp_test_x, ignore_index=True)
    test_y = target_test_y._append(comp_test_y, ignore_index=True)

    #Can comprise a val set of different sizes to aid in model evaluation during trianing (can be used for model selection)
    val_x = target_test_x._append(comp_test_x.sample(frac=1).reset_index(drop=True))
    val_y = target_test_y._append(comp_test_y.sample(frac=1).reset_index(drop=True))
    ...
    save_path = PATH_TO_DATA + "ImaginedSpeechData\\" + subject + '\\DenseNet\\' + im_type + '\\' + sum_diff + "\\" + target + "_model" + ".keras"

    if not os.path.exists(save_path):
        os.mkdir(save_path)

    #Set model save path
    save_model = save_path + '\\' + target + '_model' + ".keras"
